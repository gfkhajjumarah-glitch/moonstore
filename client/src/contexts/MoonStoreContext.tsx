...
export function MoonStoreProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<MoonStoreContextValue>(() => {
    ...
    return {
      ...
      addBlock: (type) => { const id = makeId("block"); set((draft) => { draft.store.blocks.push({ id, type, visible: true, title: type === "text" ? "New section" : type === "service" ? "Work with me" : type === "course" ? "Learn with me" : type === "booking" ? "Book a session" : undefined, body: type === "text" ? "Share something useful with your audience." : type === "service" ? "A focused way to work together." : type === "course" ? "A practical path to your next milestone." : type === "booking" ? "Choose a time that works for you." : undefined, buttonLabel: type === "button" ? "Explore" : type === "booking" ? "Choose a time" : "Get started", price: ["service", "course", "booking"].includes(type) ? 0 : undefined, currency: ["service", "course", "booking"].includes(type) ? "PKR" : undefined, availability: ["service", "course", "booking"].includes(type) ? "Open for bookings" : undefined, schedulingUrl: type === "booking" ? "" : undefined, productIds: type === "course" ? [] : undefined, whatsappNumber: type === "whatsapp" ? "" : undefined, whatsappMessage: type === "whatsapp" ? "Hi! I would love to learn more." : undefined, productId: type === "product" ? draft.products.find((item) => item.status === "published")?.id : undefined }); }); return id; },
